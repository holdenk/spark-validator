...
class ValidationTests extends FunSuite with SharedSparkContext {
  ...
  test("records read test") {
    ...
    sc.textFile("./README.md").map(_.length).
      saveAsTextFile(Files.createTempDir().toURI().toString() + "/magic")
    assert(validator.validate() === true)
  }
  ...
  test("random task failure test") {
    ...
    input.saveAsTextFile(
      Files.createTempDir().toURI().toString() + "/magic")
    assert(validator.validate() === true)
  }
  ...
  private def runTwoCounterJob(
    sc: SparkContext, valid: Accumulator[Int], invalid: Accumulator[Int]) {
    val input = sc.parallelize(1.to(10), 5)
